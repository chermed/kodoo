...
func getServerInfo(options *Options) *tview.Grid {
	...
	if err != nil {
		addField(options, infos, 0, 0, "Error", err.Error(), true)
	} else {
		addField(options, infos, 0, 0, "Host", currentConfig.Host, false)
		addField(options, infos, 1, 0, "Database", currentConfig.Database, false)
		addField(options, infos, 2, 0, "User", currentConfig.User, false)
		if options.Config.MetaConfig.NoPassword {
			addField(options, infos, 3, 0, "Password", "*****", false)
		} else {
			addField(options, infos, 3, 0, "Password", currentConfig.Password, false)
		}
	}
	return infos
}
